...
**Note 1 — ④ is the shared content *vocabulary available*, and its LOOK has a single reference (OQ-1, resolved 2026-06-13).** Both consoles compose ①②③⑤ identically. ④ is the shared vocabulary; **which screens use it is each app's call** — Cortex's content is still bespoke (admin tables + `Card`/`Input`/`Label`) and adopts the shared kit (`EditableSection`/`ListRow`/`FieldGrid`/…) only on a future per-screen refactor; Paddock is fully on the shared kit. But the **content look is now one reference = Cortex's**: shared content components render to it by default — **Card shell `rounded-lg` + `border`; `Badge` `rounded` (dot opt-in, off); `SectionHeader` neutral `text-foreground`/`font-medium`/sans (tint via prop only); roster lists are tables; column `max-w-6xl`/`p-8`**. Interactive accents (links/actions) keep `--primary`. See `OPEN_QUESTIONS.md` OQ-1.
